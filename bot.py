...
def get_all_users():
    conn = sqlite3.connect('bot.db')
    cursor = conn.cursor()
    cursor.execute('SELECT user_id, username, first_name, last_name, balance FROM users ORDER BY user_id')
...
async def button_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data == "shop_categories":
        ...
        await admin_manage_balance(query, context)
    elif data == "topup_balance":
        await topup_balance(query, context)
    elif data.startswith("admin_user_"):
        user_id = int(data.split("_")[2])
        await admin_edit_user_balance(query, context, user_id)
        ...
        await show_profile(query, context)
    elif data == "back_to_balance_management":
        await admin_manage_balance(query, context)
...
async def admin_panel(query, context):
    ...
    keyboard = [
        ...
        [InlineKeyboardButton("💰 Управление балансами", callback_data="admin_manage_balance")],
        [InlineKeyboardButton("🔙 Главное меню", callback_data="start")]
    ]
    reply_markup = InlineKeyboardMarkup(keyboard)
...
async def admin_manage_balance(query, context):
    if query.from_user.id != ADMIN_CHAT_ID:
        await query.answer("У вас нет доступа!")
        return
    ...
    keyboard = []
    # Показываем первых 20 пользователей (чтобы не превысить лимит кнопок)
    for user in users[:20]:
        user_id, username, first_name, last_name, balance = user
        display_name = username or f"{first_name} {last_name or ''}".strip()
        keyboard.append([
            InlineKeyboardButton(
                f"{display_name} - {balance:.2f} руб.", 
    ...
    await query.edit_message_text(
        ...
        "Выберите пользователя для изменения баланса:",
        reply_markup=reply_markup,
        parse_mode='Markdown'
    )

async def admin_edit_user_balance(query, context, user_id: int):
    if query.from_user.id != ADMIN_CHAT_ID:
        await query.answer("У вас нет доступа!")
        return
    ...
    keyboard = [
        [InlineKeyboardButton("➕ Пополнить на 100 руб.", callback_data="admin_balance_add_100")],
        [InlineKeyboardButton("➕ Пополнить на 500 руб.", callback_data="admin_balance_add_500")],
        [InlineKeyboardButton("➖ Списать 100 руб.", callback_data="admin_balance_sub_100")],
        [InlineKeyboardButton("➖ Списать 500 руб.", callback_data="admin_balance_sub_500")],
        [InlineKeyboardButton("💵 Установить точную сумму", callback_data="admin_balance_set")],
        [InlineKeyboardButton("🔙 Назад", callback_data="admin_manage_balance")]
    ]
    reply_markup = InlineKeyboardMarkup(keyboard)
...
async def handle_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if user_id == ADMIN_CHAT_ID and context.user_data.get('adding_category'):
        ...
        await admin_manage_categories_from_message(update, context)
        return
    
    # Обработка установки точного баланса
    elif user_id == ADMIN_CHAT_ID and context.user_data.get('setting_balance'):
        user_id_to_edit = context.user_data['editing_balance_user_id']
        del context.user_data['setting_balance']
        
        try:
            new_balance = float(message_text)
            set_user_balance(user_id_to_edit, new_balance)
            
            user = get_user(user_id_to_edit)
            display_name = user[2] if user else "Пользователь"
            
            await update.message.reply_text(
                f"✅ Баланс пользователя {display_name} установлен: {new_balance:.2f} руб."
            )
            
            # Возвращаемся к управлению балансами
            await admin_manage_balance_from_message(update, context)
    ...
    # Обработка добавления товара
    elif user_id == ADMIN_CHAT_ID and context.user_data.get('adding_product'):
        ...
        if stage == 'name':
            ...
            try:
                ...
                for category in categories:
                    cat_id, name, created_at = category
                    keyboard.append([InlineKeyboardButton(name, callback_data=f"cat_select_{cat_id}")])
                
                keyboard.append([InlineKeyboardButton("❌ Без категории", callback_data="cat_select_none")])
...
# Вспомогательные функции для возврата в меню
async def admin_manage_categories_from_message(update, context):
    categories = get_categories()
...
async def admin_manage_balance_from_message(update, context):
    users = get_all_users()
...
async def handle_category_selection(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if query.data.startswith("cat_select_"):
        category_selection = query.data.split("_")[2]
        ...
        keyboard = [
            ...
            [InlineKeyboardButton("🛍️ Посмотреть в магазине", callback_data="shop_categories")]
        ]
        reply_markup = InlineKeyboardMarkup(keyboard)
        
        await query.edit_message_text(
            f"✅ Товар успешно добавлен в магазин!\n\n"
            f"📦 Название: {context.user_data.get('product_name', 'N/A')}\n"
            f"💰 Цена: {context.user_data.get('product_price', 0):.2f} руб.\n"
...
async def handle_balance_operations(update: Update, context: ContextTypes.DEFAULT_TYPE):
    query = update.callback_query
    await query.answer()
    ...
    if query.data.startswith("admin_balance_"):
        parts = query.data.split("_")
        operation = parts[2]  # add, sub, set
        user_id = context.user_data.get('editing_balance_user_id')
        
        if not user_id:
            await query.answer("Ошибка: пользователь не выбран!")
            return
        ...
        if operation == "set":
            ...
            await query.edit_message_text(
                ...
                "Введите новую сумму баланса (в рублях):"
            )
            return
        
        # Для операций add/sub получаем сумму из callback data
        if len(parts) > 3:
            ...
            if operation == "add":
                ...
                operation_text = f"пополнен на {amount:.2f} руб."
            elif operation == "sub":
                update_user_balance(user_id, -amount)
                new_balance = max(0, user[4] - amount)  # Не даем уйти в минус
                operation_text = f"списано {amount:.2f} руб."
            
            display_name = user[2] or "Пользователь"
            
            keyboard = [[InlineKeyboardButton("🔙 Назад", callback_data=f"admin_user_{user_id}")]]
            reply_markup = InlineKeyboardMarkup(keyboard)
            
            await query.edit_message_text(
                f"✅ Баланс пользователя {display_name} {operation_text}\n"
...
def main():
    ...
    application.add_handler(CallbackQueryHandler(button_handler, pattern="^(shop_categories|profile|help|admin_panel|admin_stats|admin_broadcast|admin_add_product|admin_manage_categories|admin_manage_balance|topup_balance|back_to_shop|back_to_admin|back_to_profile|back_to_balance_management)"))
    application.add_handler(CallbackQueryHandler(handle_category_selection, pattern="^cat_select_"))
    application.add_handler(CallbackQueryHandler(handle_balance_operations, pattern="^admin_balance_"))
    application.add_handler(CallbackQueryHandler(button_handler, pattern="^(topup_|topup_pay_|category_|product_|buy_|pay_|check_|admin_delete_category_|admin_user_|admin_add_category).*"))
